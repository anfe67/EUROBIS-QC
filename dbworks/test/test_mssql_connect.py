...
class Test(TestCase):
    ...
    def test_retrieve_sample_dataset(self):
        ...
        cursor2.execute(f"SELECT  eurobis_measurementorfact.* from eurobis join eurobis_measurementorfact on eurobis.occurrenceID = "
                        f"eurobis_measurementorfact.occurrenceID where eurobis.CollectionCode ='BayOfPuck'")
